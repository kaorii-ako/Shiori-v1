...
import { useAuthStore, useAssignmentsStore, useNotesStore, useXPStore } from '../stores'
...
const BADGES = [
  ...
  { icon: '🎯', label: 'Focused', desc: 'Used Focus Mode', req: () => false },
]
...
export default function Profile() {
  ...
  const stats = { streak, notes: noteCount, completed, xp, level }
    ...
    <div style={{ fontFamily: "'Manrope', sans-serif", color: C.text, maxWidth: 700, margin: '0 auto' }}>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: '28px 28px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 24 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 13, color: C.textMuted, marginTop: 2 }}>{user?.email || 'Demo User'}</div>
